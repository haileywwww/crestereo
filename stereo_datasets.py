...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/nas/FlyingThings3D_release/', dstype='frames_finalpass', things_test=False):
    ...
    def _add_things(self, split='TRAIN'):
        ...
        root = self.root
        path = osp.join(root, self.dstype, split, '**/left/*.png')
        left_images = sorted( glob(path,recursive=True) )
...
class UnrealStereo(StereoDataset):
    def __init__(self, aug_params=None, root='/data2/unrealstereo_10k_clean', split='F'):
        super(UnrealStereo,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispUnrealStereo)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, '**/imL/*.png')))
        image2_list = sorted(glob(os.path.join(root, '**/imR/*.png')))
        disp_list = sorted(glob(os.path.join(root, '**/dispL_occ_pfm/*.pfm'))) 
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]

class CREStereo(StereoDataset):
    def __init__(self, aug_params=None, root='/data2/zhouqing/CREStereo/stereo_trainset/crestereo', split='F'):
        super(CREStereo,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispCREStereo)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, '**/*_left.jpg')))[:50000]
        image2_list = sorted(glob(os.path.join(root, '**/*_right.jpg')))[:50000]
        disp_list = sorted(glob(os.path.join(root, '**/*left.disp.png')))[:50000]
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]

class InStereo2K(StereoDataset):
    def __init__(self, aug_params=None, root='/data3/InStereo2K', split='F'):
        super(InStereo2K,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispInStereo2K)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, '**/*/left.png')))
        image2_list = sorted(glob(os.path.join(root, '**/*/right.png')))
        disp_list = sorted(glob(os.path.join(root, '**/*/left_disp.png'))) 
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]

class SimSIN(StereoDataset):
    def __init__(self, aug_params=None, root='/data3/SimSIN', split='F'):
        super(SimSIN,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispSimSIN)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, '**/*0.png'),recursive=True))
        image2_list = sorted(glob(os.path.join(root, '**/*1.png'),recursive=True))
        disp_list = sorted(glob(os.path.join(root, '**/*0_depth.npy'),recursive=True)) 
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]

class VA(StereoDataset):
    def __init__(self, aug_params=None, root='/data3/VA', split='F'):
        super(VA,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispVA)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, 'camera_0/*.png')))
        image2_list = sorted(glob(os.path.join(root, 'camera_1/*.png')))
        disp_list = sorted(glob(os.path.join(root, 'gt_depth_camera_0/*depth.pfm'))) 
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]

class IRS(StereoDataset):
    def __init__(self, aug_params=None, root='/nas/IRS', split='F'):
        super(IRS,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispIRS)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, '**/l*.png'),recursive=True))
        image2_list = sorted(glob(os.path.join(root, '**/r*.png'),recursive=True))
        disp_list = sorted(glob(os.path.join(root, '**/d*.exr'),recursive=True)) 
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]

class UnrealCV(StereoDataset):
    def __init__(self, aug_params=None, root='/nas/dataset_unrealcv', split='F'):
        super(UnrealCV,self).__init__(aug_params, sparse=False, reader=frame_utils.readDispUnrealCV)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, '**/left*.png'),recursive=True))
        image2_list = sorted(glob(os.path.join(root, '**/right*.png'),recursive=True))
        disp_list = sorted(glob(os.path.join(root, '**/disparity*.txt'),recursive=True)) 
        for img1, img2, disp in zip(image1_list, image2_list, disp_list):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if re.compile("middlebury_.*").fullmatch(dataset_name):
            ...
            final_dataset = SceneFlowDatasets(aug_params, dstype='frames_cleanpass')
        ...
        elif dataset_name == 'unreal_stereo':
            new_dataset = UnrealStereo(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from UnrealStereo")
        ...
        elif dataset_name == 'simsin':
            new_dataset = SimSIN(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from SimSIN")
        elif dataset_name == 'va':
            new_dataset = VA(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from VA")
        elif dataset_name == 'irs':
            new_dataset = IRS(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from IRS")
        elif dataset_name == 'unrealcv':
            new_dataset = UnrealCV(aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from UnrealCV")
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, 
        pin_memory=True, num_workers=4, drop_last=True, sampler=train_sampler)
