...
def readDispUnrealStereo(file_name):
    disp = readPFM(file_name).astype(np.float32)
    assert len(disp.shape) == 2
    fname = file_name.replace('dispL_occ_pfm', 'occlusion_mask').replace('pfm','png')
    mask = np.array(Image.open(fname))
    valid = ((mask == 0) & (disp > 0))
    return disp, valid

def readDispCREStereo(file_name):
    disp = cv2.imread(file_name,-1).astype(np.float32) / 32    
    assert len(disp.shape) == 2
    valid = disp > 0
    return disp,valid

def readDispInStereo2K(file_name):
    disp = cv2.imread(file_name,-1).astype(np.float32) / 100    
    assert len(disp.shape) == 2
    valid = disp > 0
    return disp,valid
...
def readDispSimSIN(file_name):
    depth = np.load(file_name)   
    disp = depth.copy()
    disp[disp!=0] = 33.28/depth[disp!=0]
    assert len(disp.shape) == 2
    valid = disp > 0
    return disp,valid
...
def readDispIRS(file_name):
    disp = cv2.imread(file_name, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[:,:,1]
    assert len(disp.shape) == 2
    valid = disp > 0
    return disp,valid

def readDispUnrealCV(file_name):
